...
### Hamburg ####
# keine/ungenügend erhobenen Daten in den Jahren 2011, 2012, 2013, 2015, 2018, 2019, 2020, 2021
#### 2014 ####
problems_2014_hh_raw <- raw_2014 %>% select(Bundesland, Problemlage..sozialrechtliche.Probleme, Problemlage..Umgang.mit.Behörden, Problemlage..Arbeitslosigkeit,
                                            Problemlage..Sanktionen.nach.SGB.II, Problemlage..Schulden, Problemlage..Energie..oder.Mietschulden,
                                            Problemlage..sontige.finanzielle.Prob..., Problemlage..Partner...Erziehung..fam...,
                                            Problemlage..Psychische.Probleme, Problemlage..Krankheit, Problemlage..Alter.Pflegebedürftigkeit,
                                            Problemlage..Schwangerschaft, Problemlage..Behinderung, Problemlage..Bereich.Wohnen, Problemlage..Sonstiges) %>%
...
problems_2014_hh <- rbind(
  
  problems_2014_hh_raw %>% select(Problemlage = Problemlage..sozialrechtliche.Probleme) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "sozialrechtliche Probleme")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2014_hh_raw %>% select(Problemlage = Problemlage..Partner...Erziehung..fam...) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Partner/Erziehung/familiäre Probleme")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2014_hh_raw %>% select(Problemlage = Problemlage..Umgang.mit.Behörden) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Umgang mit Behörde")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2014_hh_raw %>% select(Problemlage = Problemlage..Psychische.Probleme) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "psychische Probleme")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2014_hh_raw %>% select(Problemlage = Problemlage..Arbeitslosigkeit) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Arbeitslosigkeit")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2014_hh_raw %>% select(Problemlage = Problemlage..Krankheit) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Krankheit")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2014_hh_raw %>% select(Problemlage = Problemlage..Sanktionen.nach.SGB.II) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Sanktionen nach SGB II")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2014_hh_raw %>% select(Problemlage = Problemlage..Alter.Pflegebedürftigkeit) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Alter/Pflegebedürftigkeit")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2014_hh_raw %>% select(Problemlage = Problemlage..Schulden) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Schulden")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2014_hh_raw %>% select(Problemlage = Problemlage..Schwangerschaft) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Schwangerschaft")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2014_hh_raw %>% select(Problemlage = Problemlage..Energie..oder.Mietschulden) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Energie- oder Mietschulden")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2014_hh_raw %>% select(Problemlage = Problemlage..Behinderung) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Behinderung")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2014_hh_raw %>% select(Problemlage = Problemlage..sontige.finanzielle.Prob...) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Sonstige finanzielle Schwierigkeiten")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2014_hh_raw %>% select(Problemlage = Problemlage..Bereich.Wohnen) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Probleme im Bereich Wohnen")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2014_hh_raw %>% select(Problemlage = Problemlage..Sonstiges) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage != ''), 1)) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Sonstiges")) %>%
    count(Problemlage) %>%
    arrange(desc(Problemlage)) %>%
...
# Korrektur: Prozent
problems_2014_hh <- problems_2014_hh %>% mutate(Prozent = round((n/(sum(n)))*100,2))

#### 2016 ####

problems_2016_hh_raw <- raw_2016 %>% select(Bundesland, Problemlage..sozialrechtliche.Probleme, Problemlage..Umgang.mit.Behörden, Problemlage..Arbeitslosigkeit,
                                            Problemlage..Sanktionen.nach.SGB.II, Problemlage..Schulden, Problemlage..Energieschulden, Problemlage..Mietschulden,
                                            Problemlage..sontige.finanzielle.Prob..., Problemlage..Partner...Erziehung..fam...,
                                            Problemlage..Psychische.Probleme, Problemlage..Krankheit, Problemlage..Alter.Pflegebedürftigkeit,
                                            Problemlage..Schwangerschaft, Problemlage..Behinderung, Problemlage..Bereich.Wohnen, Problemlage..Sonstiges) %>%
...
problems_2016_hh <- rbind(
  
  problems_2016_hh_raw %>% select(Problemlage = Problemlage..sozialrechtliche.Probleme) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "sozialrechtliche Probleme")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2016_hh_raw %>% select(Problemlage = Problemlage..Partner...Erziehung..fam...) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Partner/Erziehung/familiäre Probleme")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2016_hh_raw %>% select(Problemlage = Problemlage..Umgang.mit.Behörden) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Umgang mit Behörde")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2016_hh_raw %>% select(Problemlage = Problemlage..Psychische.Probleme) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "psychische Probleme")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2016_hh_raw %>% select(Problemlage = Problemlage..Arbeitslosigkeit) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Arbeitslosigkeit")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2016_hh_raw %>% select(Problemlage = Problemlage..Krankheit) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Krankheit")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2016_hh_raw %>% select(Problemlage = Problemlage..Sanktionen.nach.SGB.II) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Sanktionen nach SGB II")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2016_hh_raw %>% select(Problemlage = Problemlage..Alter.Pflegebedürftigkeit) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Alter/Pflegebedürftigkeit")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2016_hh_raw %>% select(Problemlage = Problemlage..Schulden) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Schulden")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2016_hh_raw %>% select(Problemlage = Problemlage..Schwangerschaft) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Schwangerschaft")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2016_hh_raw %>% select(Problemlage = Problemlage..Energieschulden) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Energieschulden")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2016_hh_raw %>% select(Problemlage = Problemlage..Mietschulden) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Mietschulden")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2016_hh_raw %>% select(Problemlage = Problemlage..Behinderung) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Behinderung")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2016_hh_raw %>% select(Problemlage = Problemlage..sontige.finanzielle.Prob...) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Sonstige finanzielle Schwierigkeiten")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2016_hh_raw %>% select(Problemlage = Problemlage..Bereich.Wohnen) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Probleme im Bereich Wohnen")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2016_hh_raw %>% select(Problemlage = Problemlage..Sonstiges) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage != ''), 1)) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Sonstiges")) %>%
    count(Problemlage) %>%
    arrange(desc(Problemlage)) %>%
...
problems_2016_hh[8,1] <- "Alter/Pflegebedürftigkeit"
problems_2016_hh[8,2] <- 0
problems_2016_hh[8,3] <- 0.00
problems_2016_hh[9,1] <- "Schulden"
problems_2016_hh[9,2] <- 0
problems_2016_hh[9,3] <- 0.00

#### 2017 ####

problems_2017_hh_raw <- raw_2017 %>% select(Bundesland, Problemlage..sozialrechtliche.Probleme, Problemlage..Umgang.mit.Behörden, Problemlage..Arbeitslosigkeit,
                                            Problemlage..Sanktionen.nach.SGB.II, Problemlage..Schulden, Problemlage..Energieschulden, Problemlage..Mietschulden,
                                            Problemlage..sontige.finanzielle.Prob..., Problemlage..Partner...Erziehung..fam...,
                                            Problemlage..Psychische.Probleme, Problemlage..Krankheit, Problemlage..Alter.Pflegebedürftigkeit,
                                            Problemlage..Schwangerschaft, Problemlage..Behinderung, Problemlage..Bereich.Wohnen, Problemlage..Sonstiges) %>%
...
problems_2017_hh <- rbind(
  
  problems_2017_hh_raw %>% select(Problemlage = Problemlage..sozialrechtliche.Probleme) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "sozialrechtliche Probleme")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2017_hh_raw %>% select(Problemlage = Problemlage..Partner...Erziehung..fam...) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Partner/Erziehung/familiäre Probleme")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2017_hh_raw %>% select(Problemlage = Problemlage..Umgang.mit.Behörden) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Umgang mit Behörde")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2017_hh_raw %>% select(Problemlage = Problemlage..Psychische.Probleme) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "psychische Probleme")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2017_hh_raw %>% select(Problemlage = Problemlage..Arbeitslosigkeit) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Arbeitslosigkeit")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2017_hh_raw %>% select(Problemlage = Problemlage..Krankheit) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Krankheit")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2017_hh_raw %>% select(Problemlage = Problemlage..Sanktionen.nach.SGB.II) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Sanktionen nach SGB II")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2017_hh_raw %>% select(Problemlage = Problemlage..Alter.Pflegebedürftigkeit) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Alter/Pflegebedürftigkeit")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2017_hh_raw %>% select(Problemlage = Problemlage..Schulden) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Schulden")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2017_hh_raw %>% select(Problemlage = Problemlage..Schwangerschaft) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Schwangerschaft")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2017_hh_raw %>% select(Problemlage = Problemlage..Energieschulden) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Energieschulden")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2017_hh_raw %>% select(Problemlage = Problemlage..Mietschulden) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Mietschulden")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2017_hh_raw %>% select(Problemlage = Problemlage..Behinderung) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Behinderung")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2017_hh_raw %>% select(Problemlage = Problemlage..sontige.finanzielle.Prob...) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Sonstige finanzielle Schwierigkeiten")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2017_hh_raw %>% select(Problemlage = Problemlage..Bereich.Wohnen) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Probleme im Bereich Wohnen")) %>%
    count(Problemlage) %>%
    mutate(Prozent = round((n/(sum(n)))*100,2)) %>%
    slice_tail(),
  
  problems_2017_hh_raw %>% select(Problemlage = Problemlage..Sonstiges) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage != ''), 1)) %>%
    mutate(Problemlage = replace(Problemlage, which(Problemlage == 1), "Sonstiges")) %>%
    count(Problemlage) %>%
    arrange(desc(Problemlage)) %>%
...
problems_2017_hh[5,1] <- "Arbeitslosigkeit"
problems_2017_hh[5,2] <- 0
problems_2017_hh[5,3] <- 0.00
problems_2017_hh[7,1] <- "Sanktionen nach SGB II"
problems_2017_hh[7,2] <- 0
problems_2017_hh[7,3] <- 0.00
problems_2017_hh[10,1] <- "Schwangerschaft"
problems_2017_hh[10,2] <- 0
problems_2017_hh[10,3] <- 0.00
problems_2017_hh[11,1] <- "Energieschulden"
problems_2017_hh[11,2] <- 0
problems_2017_hh[11,3] <- 0.00
problems_2017_hh[12,1] <- "Mietschulden"
problems_2017_hh[12,2] <- 0
problems_2017_hh[12,3] <- 0.00


#### Zusammenfassen von Kategorien #####

problems_2014_hh <- problems_2014_hh %>% 
  mutate(Problemlage = case_when(
    Problemlage %in% c("Krankheit", "psychische Probleme") ~ "Krankheit (auch psychische Probleme)",
    TRUE ~ Problemlage)) %>%
  group_by(Problemlage) %>%
...
problems_2016_hh <- problems_2016_hh %>% 
  mutate(Problemlage = case_when(
    Problemlage %in% c("Energieschulden", "Mietschulden") ~ "Energie- oder Mietschulden",
    Problemlage %in% c("Krankheit", "psychische Probleme") ~ "Krankheit (auch psychische Probleme)",
    TRUE ~ Problemlage)) %>%
...
problems_2017_hh <- problems_2017_hh %>% 
  mutate(Problemlage = case_when(
    Problemlage %in% c("Energieschulden", "Mietschulden") ~ "Energie- oder Mietschulden",
    Problemlage %in% c("Krankheit", "psychische Probleme") ~ "Krankheit (auch psychische Probleme)",
    TRUE ~ Problemlage)) %>%
...
ts_problems_hamburg <- list(problems_2014_hh, problems_2016_hh, problems_2017_hh) %>%
  reduce(left_join, by = "Problemlage")


ts_problems_hamburg <- ts_problems_hamburg %>% select(Problemlage, contains("Prozent"))
colnames(ts_problems_hamburg) <- c("Problemlage", "2014", "2016", "2017")

ts_problems_hamburg2 <- ts_problems_hamburg %>% pivot_longer(!Problemlage, 
                                                           names_to = "Jahr", 
                                                           values_to = "Prozent") %>%
  mutate(Problemlage = factor(Problemlage,
                              levels = c("Alter/Pflegebedürftigkeit", "Behinderung", 
                                         ...
                                         "Sonstige finanzielle Schwierigkeiten", "Probleme im Bereich Wohnen", 
                                         "Partner/Erziehung/familiäre Probleme", "Sonstiges")))
structure(ts_problems_hamburg2$Problemlage)

problems_hh <- ts_problems_hamburg2 %>%
  filter(Problemlage == "sozialrechtliche Probleme" |
           Problemlage == "Umgang mit Behörde" |
           Problemlage == "Energie- oder Mietschulden" |
           Problemlage == "Probleme im Bereich Wohnen") %>%
  ...
  geom_line(size = 1.1) + 
  theme_linedraw() +
  labs(title = "Problemlagen", subtitle = "Hamburg | 2014-2017") +
  theme(plot.title = element_text(size = 15, face = "bold", hjust = 0.5),
        plot.subtitle = element_text(size = 13, face = "bold", hjust = 0.5)) +
  scale_color_brewer(palette = "Paired")
