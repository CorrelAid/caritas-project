---
title: "CorrelAid: Bericht zur Auswertung der Allgemeinen Sozialberatung der Caritas"
author: "Ann-Kristin Vester, Christoph Wolf, Dylan Thurgood, Katharina Potinius, Sven Ehmes"
date: "31. Januar 2022"
lang: de-DE
output: pdf_document
header-includes:
  ...
  - \pretitle{\begin{center}
    \includegraphics[width=4in,height=4in]{images/Correlaid.png}\LARGE\\}
...
```{r Vergleich Familie SetUp, include=FALSE, warning=FALSE, message=FALSE}
...
```{r Vergleich Herkunft SetUp, include=FALSE, warning=FALSE, message=FALSE}
source("skripte/Grafiken_Repraesentativitaet_Herkunft.R", encoding = "UTF-8")
```

```{r Grafik Vergleich Herkunft, warning=FALSE, message=FALSE}
...
Grundsätzlich haben die meisten Menschen in Deutschland einen Abschluss, wobei der Anteil der Menschen mit tertiärer Bildung (Meister, Studium) über die Jahre hinweg ansteigt. Es überrascht nicht, dass die niedriger Gebildete in der Beratung überrepräsentiert sind. Die Zahlen für 2020 sind provisorisch und mehrere breaks in series fanden statt, die Zahlen der jeweiligen Jahre sind also nicht vollumfänglich untereinander vergleichbar. Quelle: EU-LSF (edat_lfse_03).
...
```{r Grafik Bildung ohne Migration, out.width="80%"}
...
```{r Grafik Bildung mit Migration, out.width="80%"}
...
problems_bb
```
Der massive Anstieg von Miet- und Energiepreisen bildet sich nur zu einem gewissen Grad in den Problemlagen ab, aufgrund deren Menschen die Sozialberatung der Caritas aufsuchen. In besonders betroffenen Regionen, hier Berlin, zeigt sich über die letzten zehn Jahre - mit sichtbaren Schwankungen - ein markanter Anstieg an wohnungsbedingter Probleme. Die drängendsten Probleme, weswegen sich Personen an die Sozialberatung wenden, liegen jedoch auch in diesen Regionen (in Einklang mit dem allgemeinen bundesweiten Trend) mit über 50% beim Umgang mit Behörden und im Bereich des Sozialrechts.Es ist jedoch in Frage zu Stellen, inwieweit Berlin mit anderen deutschen Metropolen vergleichbar ist. 

\newpage
## Statistische Analysen
Die nachfolgenden Graphiken dienen der Auswertung folgender Fragestellung: Welche Faktoren haben maßgeblichen Einfluss auf die Wahrscheinlichkeit einer bestimmten Problemlage ausgesetzt zu sein? 
\medskip 
\newline 
\noindent
Alle Analysen basieren auf den Fragebogendaten für das Jahr 2021. Um zu analysieren, welche Personen eine höhere Wahrscheinlichkeit haben, mit einer bestimmten Problemlage konfrontiert zu werden, haben wir sogenannte "Random Forest Modelle" angewandt. Diese ermöglichen eine Einschätzung der relativen Variablenwichtigkeit. Denjenigen Variablen, die einen größeren „Mean Decrease in Accuracy“ ermöglichen, d.h. die abhängige Variable besser erklären, wird eine höhere relative Variablenwichtigkeit zugesprochen. 
\medskip 
\newline 
\noindent 
Das Random Forest Modell sagt jedoch nichts über das Vorzeichen (+/-) der Effekte aus. Ein Beispiel: Nehmen wir an, dass laut des Random Forest Modells das Alter der wichtigste Faktor, hinsichtlich des Vorliegens einer bestimmten Problemlage, ist. Trotz dieser Erkenntnis wissen wir nicht, ob jüngere oder ältere Menschen eher von dieser Problemlage betroffen sind. Daher ziehen wir ein logistisches Regressionsmodell hinzu. Das Vorzeichen der resultierenden Koeffizienten gibt die Richtung des Effektes an. Wir haben dieses Vorzeichen in den Graphiken in Klammern hinter der entsprechenden Variable notiert, sofern der Effekt statistisch signifikant war. In diesen Fällen haben wir ebenfalls das Signifikanzniveau notiert. 
\newline
* bedeutet ein Signifikanzniveau von 10%, 

\newline
** bedeutet ein Signifikanzniveau von 5% und 

\newline
*** bedeutet ein Signifikanzniveau von 1%. 

\newline
Je mehr Sterne, desto signifikanter ist der Effekt. Hat ein Effekt beispielsweise ein Signifikanzniveau von 10%, bedeutet dies, dass wir mit 90%iger Sicherheit davon ausgehen können, dass der Faktor einen signifikanten Einfluss auf die Wahrscheinlichkeit einer bestimmten Problemlage hat, während wir dies bei einem Signifikanzniveau von 1% mit 99%iger Sicherheit schlussfolgern können. 
...
Abbildung 14 analysiert, welche Personen mit einer höherer Wahrscheinlichkeit von Problemen mit Behörden betroffen sind. Es überrascht nicht, dass vor allem Menschen mit Migrationshintergrund, ohne Abschluss, Alleinstehende, Alleinerziehende sowie Menschen mit vergleichsweiße mehr Kindern betroffen sind (alle jeweils Signifikanzniveau von 1%). Darüber hinaus scheinen behinderte und, überraschenderweise, gesunde Menschen tendenziell eher Probleme mit den Behörden zu haben (Signifikanzniveau von 5%). Nach Alter und Geschlecht ergeben sich keine statistisch signifikante Unterschiede.
...
Abbildung 16 analysiert, welche Personen eine höhere Wahrscheinlichkeit haben, bei der Antragstellung im Jobcentre auf Probleme zu stoßen. Dies sind Menschen mit mehr Kindern (Signifikanzniveau von 5%), jüngere Menschen, Männer und Menschen ohne Abschluss (Signifikanzniveau von 1%) sowie alleinstehende und alleinerziehende Menschen (Signifikanzniveau von 5%). Überraschenderweise finden sich keine signifikanten Unterschiede nach Gesundheitsstatus und Migrationshintergrund.
...
Abbildung 18 analysiert, welche Personen eine höhere Wahrscheinlichkeit aufweisen, verschuldet zu sein. Dies sind vor allem Männer und Menschen ohne Migrationshintergrund (jeweils Signifikanzniveau von 1%) sowie Menschen ohne Abschluss (Signifikanzniveau von 10%). Das Alter, der Familienstand und die Anzahl der Kinder scheinen dagegen kaum Einfluss auf die Schuldenlage zu haben. 
...
Abbildung 19 analysiert, bei welchen Personen sich die finanzielle Situation infolge der Corona-Pandemie tendenziell verschlechtert hat. Dies sind Menschen mit Migrationshintergrund (1% Signifikanzniveau), Alleinstehende und Alleinerziehende,  Menschen mit Behinderung sowie Menschen mit Abschluss (jeweils 5% Signifikanzniveau). Dagegen wurden keine signifikanten Unterschiede nach Alter, Geschlecht und Krankheit gefunden.
